...
The David City display uses an eastern-Nebraska radar viewport (`101°W–94.2°W`, `39.85°N–42.8°N`) to provide room around the office while retaining nearby Iowa, Kansas, and South Dakota context. The IEM source raster and overlay geometry are both WGS84/EPSG:4326, so state/county/city features are projected into that exact extent. It first resolves NWS point `41.2528,-97.1301`, which NWS identifies as David City, NE, then follows that response's `forecast`, `forecastHourly`, and `observationStations` URLs. The forecast panel autonomously cross-fades every 20 seconds between three large hourly slots and four daily slots. Hourly signage uses the next full local hour, filtering out periods already ended or in progress; each precipitation chance remains tied to that NWS period's exact displayed hour. NWS icon URLs are rendered directly with a visible `NWS` fallback if an icon cannot load. Only overflowing condition text slowly pauses and scrolls; reduced-motion settings disable it. It selects the nearest official station from the point response, displays its name/ID/distance and observation time, and refreshes NWS data every ten minutes.
